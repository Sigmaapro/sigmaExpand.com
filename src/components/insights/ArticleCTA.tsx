...
import { localeBody, localeCta, localeHeading } from "@/lib/localeTypography";
  ...
  return (
    <div className="my-10 border-y border-white/[0.06] bg-[#080a0f]/40 py-10 sm:my-16 sm:py-14">
      <div className="mx-auto flex max-w-2xl flex-col items-center px-3 text-center sm:px-4">
        <p className={`font-display text-lg font-semibold leading-tight text-[#f1f3f5] text-balance sm:text-xl md:text-2xl ${localeHeading(lang)}`}>
          {heading}
        </p>
        <p className={`mt-3 max-w-xl text-sm leading-relaxed text-[#adb5bd] text-pretty sm:text-base ${localeBody(lang)}`}>
          {supporting}
        </p>
