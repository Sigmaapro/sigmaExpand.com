...
/**
 * Official React Bits Ferrofluid — adapted for Next.js App Router.
 * Source: https://reactbits.dev/backgrounds/ferrofluid
 */
export default function Ferrofluid({
  ...
  useEffect(() => {
    ...
    const renderer = new Renderer({
      dpr: dpr ?? (typeof window !== "undefined" ? Math.min(window.devicePixelRatio || 1, 1.75) : 1),
      alpha: true,
      antialias: true,
    });
    ...
    const { arr, count, avg } = prepColors(colorsKey.split(","));

    const uniforms = {
      iResolution: { value: [gl.drawingBufferWidth, gl.drawingBufferHeight, 1] as number[] },
    ...
    // Listen on section parent via window so canvas can stay pointer-events:none
    if (mouseInteraction) {
    ...
    const loop = (t: number) => {
      ...
      if (!pausedRef.current && programRef.current && meshRef.current) {
        try {
          ...
          /* ignore transient GL errors during teardown */
        }
      }
    };
    rafRef.current = requestAnimationFrame(loop);
  ...
  return (
    <div
      ref={containerRef}
      className={`ferrofluid-container pointer-events-none absolute inset-0 h-full w-full overflow-hidden ${className ?? ""}`}
      style={{
        ...(mixBlendMode
