...
export interface GradientBlindsProps {
  ...
  /** Multiplier for shader time (1 = default; <1 = slower). */
  timeScale?: number;
}
...
`;
    ...
    const pointerTarget: HTMLElement | Window = trackPointer === 'window' ? window : canvas;
    pointerTarget.addEventListener('pointermove', onPointerMove as EventListener);

    const loop = (t: number) => {
      rafRef.current = requestAnimationFrame(loop);
      uniforms.iTime.value = t * 0.001 * timeScale;
      if (mouseDampening > 0) {
