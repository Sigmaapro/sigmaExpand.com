...
export interface MagicRingsProps {
  ...
  parallax?: number;
  clickBurst?: boolean;
  /** Freeze animation (prefers-reduced-motion). */
  paused?: boolean;
  className?: string;
}

/**
 * Official React Bits Magic Rings (TS) — adapted for Next.js.
 * Source: https://reactbits.dev/animations/magic-rings
 */
export default function MagicRings({
  ...
  useEffect(() => {
    ...
    // Listen on window only when followMouse — mount is pointer-events none by design
    if (followMouse) {
    ...
    const animate = (t: number) => {
      ...
      if (p.paused) {
        if (pausedTimeRef.current === 0) pausedTimeRef.current = t;
      } else {
      ...
      const timeMs = p.paused ? pausedTimeRef.current || t : t;
      uniforms.uTime.value = timeMs * 0.001 * (p.paused ? 0 : p.speed);
      // When paused, hold a mid-cycle readable ring
      if (p.paused) {
        uniforms.uTime.value = 1.2;
        uniforms.uScaleRate.value = 0;
      } else {
      ...
      uniforms.uNoiseAmount.value = p.paused ? 0 : p.noiseAmount;
    ...
    return () => {
      ...
      if (clickBurst) mount.removeEventListener("click", onClick);
      if (canvas.parentElement === mount) mount.removeChild(canvas);
      geometry.dispose();
      material.dispose();
      renderer.dispose();
    };
  }, [followMouse, clickBurst]);
