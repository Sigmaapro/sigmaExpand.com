...
export type RippleGridProps = {
  ...
  /** Multiplier for shader time (1 = default; 0.65 ≈ 35% slower). */
  timeScale?: number;
...
/**
 * Official React Bits Ripple Grid — adapted for Next.js App Router.
 * Source: https://reactbits.dev/backgrounds/ripple-grid
 */
...
void main() {
    ...
    const render = (t: number) => {
      if (!pausedRef.current) {
        frozenTimeRef.current = t * 0.001 * timeScaleRef.current;
      }
    ...
    // Mount once; live props sync via second effect + refs.
  ...
  return (
    <div
      ref={containerRef}
      className={`ripple-grid-container h-full w-full overflow-hidden ${className}`.trim()}
      aria-hidden
    />
  );
}
