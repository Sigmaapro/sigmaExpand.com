...
export function ServicesPageView() {
  ...
  const serviceRouteKeys: ServiceDetailKey[] = [
    "platformGrowth",
    "kolInfrastructure",
    "ibProgram",
    "traderNetwork",
  ...
  const serviceRouteHrefs: Record<ServiceDetailKey, string> = {
    platformGrowth: ROUTES.servicesPlatformGrowth,
    kolInfrastructure: ROUTES.servicesKolInfrastructure,
    ibProgram: ROUTES.servicesIbProgram,
    traderNetwork: ROUTES.servicesTraderNetwork,
    tokenLaunch: ROUTES.servicesTokenLaunch,
  };
        ...
        <section className="mt-8 rounded-2xl border border-white/[0.08] bg-[#07090f]/65 p-6 shadow-[0_16px_48px_rgba(0,0,0,0.35)] backdrop-blur-md sm:p-8">
          ...
          <ul className="mt-4 grid gap-2.5 sm:grid-cols-2">
            {serviceRouteKeys.map((key) => {
              const item = getServiceDetailContent(key, language);
              return (
                <li key={serviceRouteHrefs[key]}>
                  <Link
                    href={serviceRouteHrefs[key]}
